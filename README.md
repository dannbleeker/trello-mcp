...
- **Staleness, per zone**: no card on the board carries a due date, so age is the live signal. A card shows `untouched N` once it passes its zone's threshold — Waiting-for and Inbox at 7 days (the same fuse the weekly review uses), next actions at 14, big rocks always. Red at 21 / 30 / 90 respectively. Outside big rocks a healthy board shows none of them.
